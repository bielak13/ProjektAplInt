...
class BuildsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create build" do
    assert_difference('Build.count') do
      post builds_url, params: { build: { city: @build.city, number: @build.number, street: @build.street } }
    end
  ...
  test "should update build" do
    patch build_url(@build), params: { build: { city: @build.city, number: @build.number, street: @build.street } }
